...
{
    public class ManyToManyQueueManager : BaseQueueManager
    {
        ...
        {
            actionPoint = _actionPoints.FirstOrDefault(p => !p.Occupied);
            return actionPoint != null;
        }

        private bool PickAQueuePoint(out QueuePoint actionPoint)
        {
            actionPoint = _queuePoints.FirstOrDefault(p => !p.Occupied);
            return actionPoint != null;
        }
        ...
        {
            return _actionPoints.Contains(candidate.QueuePoint);
        }
        public override bool HasFreePositions()
        {
            return _queuePoints.Exists(qp => !qp.Occupied) || _actionPoints.Exists(qp => !qp.Occupied);
        }
